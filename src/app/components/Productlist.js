...
export const Productlist = ({ heading, fruitData }) => {
    ...
    return (
        <>
            <section
                className="py-10 px-4 md:px-10 bg-cover">
                <div className='mx-auto max-w-7xl px-4 md:px-8 lg:px-2 '>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {visibleItems.map((item, index) => {
                            ...
                            return item.link ? (
                                <Link href={item.link} key={index} className="overflow-hidden block">
                                    {content}
                                </Link>
                            ) : (
                                <div key={index} className="overflow-hidden block cursor-default">
                                    {content}
                                </div>
                            );
                        })}
