...
export const EmpoweringBanner = () => {
    ...
    return (
        <section className="bg-cover bg-center relative h-[500px]  flex items-center justify-center" style={{ backgroundImage: `url(${banner.src})` }}>
            <div className='bg-black bg-opacity-30 w-[90%] lg:w-3/5 text-white text-center p-6 py-10'>
                <h2 className='redhat mb-4 text lg:mb-7 text-center text-3xl lg:text-4xl lg:text-[40px] font-semibold'>{heading}</h2>
                <p className='font-light poppins text-base md:text-[17px]'>{text}</p>
            </div>
        </section>
    )
}
            ...
            <section
                ...
                <div className='mx-auto max-w-7xl px-4 md:px-6 lg:px-2 '>
                    <div className='mx-auto justify-between flex flex-wrap lg:flex-nowrap gap-5 lg:gap-5'>
                        {box.map((item, index) => (
                            <div key={index} className='w-[47%] lg:w-3/12 text-black text-center grid items-center justify-center p-4 h-52 lg:h-36 border-[1px] border-[#1B453C] hover:bg-[#1B453C] hover:text-white rounded-[20px] transition-all  duration-400'>
                                <h2 className="lg:leading-snug  poppins text-center text-base lg:text-[17px]  font-normal  w-full">{item.text}</h2>
