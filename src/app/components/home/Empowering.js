...
export const EmpoweringBanner = ({props}) => {
    const heading = props?.data?.data?.sec11_title??'Building Natural Resources for the Generations to Come';
    const text = props?.data?.data?.sec11_content??"Our agroforestry initiatives have already made a significant positive impact on the environment and the local community."
    return (
        <section className="bg-cover bg-center relative h-[500px]  flex items-center justify-center" style={{ backgroundImage: `url(${process.env.NEXT_PUBLIC_SITEASSET+'/homepage/'+props?.data?.data?.sec11_image??banner.src})` }}>
