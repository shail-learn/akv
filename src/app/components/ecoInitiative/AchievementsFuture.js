    ...
    <>
      ...
      <section className='relative text-center md:text-left mt-10 md:mt-20 pb-16 pt-16 lg:pb-20 lg:pt-20' style={{ background: `url(${impactbg.src})center center / cover no-repeat` }} >
        <div className="lg:w-12/12  w-full  w-12/12">
          <h2 className="text-3xl md:leading-[1.3] md:text-4xl lg:text-[40px] font-semibold redhat text-center text-white mb-2">{heading}</h2>
          <h5 className="text-[22px] md:leading-[1.3] md:text-[26px] font-medium redhat text-center text-[#F7C35F] mb-3">{subtitle}</h5>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10 mt-10 w-full">
            {achievements.map((item, index) => (
              <div key={index} className="relative  overflow-hidden w-full">
                ...
                <div className="absolute left-5 top-2/4 -translate-y-2/4 bg-white bg-opacity-80 p-4 md:p-6 md:py-10 rounded-md w-[70%] md:w-[40%]">
                  <h3 className="text-xl md:text-[22px] redhat font-semibold text-[#1B453C] mb-2">{item.title}</h3>
                  <p className="text-base md:text-[17px] redhat text-[#1B453C]">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="text-center md:text-left  pb-16 pt-16 lg:pb-20 lg:pt-20">
        <div className="mx-auto max-w-7xl  px-6 flex flex-col-reverse md:flex-row items-center gap-10">
          <div className="w-full md:w-1/2 text-center md:text-left">
            ...
            <p className="text-[22px] md:leading-[1.3] md:text-[26px] font-medium redhat text-[#F7C35F] lg:mb-16 mb-10">
              {subtitle}
            </p>
            ...
            <div className="mt-16 md:mt-20">
              <Link
                href="/contact-us"
                className="bg-[#1B453C] text-white px-10 redhat py-4 rounded-[4px] text-base font-medium hover:bg-black"
              >
                JOIN THE MOVEMENT
              </Link>
            </div>
