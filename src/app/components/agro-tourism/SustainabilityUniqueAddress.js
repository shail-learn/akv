        ...
        <div className='mx-auto max-w-7xl px-4 md:px-6 lg:px-2'>
          ...
          <div className='flex flex-col lg:flex-row justify-between  gap-4 lg:gap-12'>
            <div className='lg:w-6/12  w-full w-12/12'>
              ...
              <p className='text-black text-base md:text-[17px] text-justify redhat pb-8'>{subtitle}</p>
              {agroSustainability1.map((item, index) => (
                ...
                >
                  ...
                  <div className="absolute top-6 rounded-br-[50px] left-6 bg-white bg-opacity-90 md:w-[48%] w-2/3  p-6 flex flex-col">
                    <h3 className="text-xl md:text-2xl font-medium text-[#1B453C]">{item.title}</h3>
                    <p className="text-[15px] md:text-[17px] text-[#1B453C] mt-2">{item.description}</p>
                  </div>
                </div>
              ))}
            ...
            <div className='lg:w-6/12  w-full w-12/12'>
              ...
              <p className='text-black text-base md:text-[17px] text-justify redhat pb-8'>{footer}</p>
    ...
    <>
      ...
      <section className='bg-[#F4F4F4] pt-12'>
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl text-center md:leading-[1.3] md:text-4xl lg:text-[40px] font-semibold redhat text-[#1B453C]">
            {heading1}
          </h2>
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8  lg:px-2 relative z-10">
        <div className="flex flex-wrap lg:flex-nowrap gap-10 md:gap-16 justify-between items-end">
          {/* Left Column */}
          <div className="w-full lg:w-6/12">
            <div className="flex flex-col h-full justify-between">
              <h2 className="text-white text-3xl md:text-4xl lg:text-[40px] font-semibold redhat mb-4 lg:mb-7 text-center lg:text-left">
                {visitInfoData.heading}
              </h2>

              <Link
                href="/contact-us"
                className="text-base font-medium rounded md:text-[17px] text-center bg-white text-black lg:mt-32 py-4 px-6 w-fit mx-auto lg:mx-0 mb-6"
              >
                {visitInfoData.buttonText}
              </Link>

              <p className="text-base md:text-[17px] text-white text-center md:text-left">
              ...
              <ul className="space-y-4">
                {visitInfoData.info.map((item, index) => {
                  ...
                  return (
                    <li
                      key={index}
                      className="text-white text-base md:text-[17px] flex items-start space-x-3"
                    >
                      <Icon className="text-[#F7C35F] mt-1" size={22} />
                      <p className="redhat">
