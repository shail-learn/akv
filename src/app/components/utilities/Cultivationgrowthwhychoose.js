  ...
  return (
    ...
    >
      ...
      <div className="relative my-2 z-10 text-white text-center px-4 max-w-4xl mx-auto GrowthNavigation">
        <h2 className="redhat  text-3xl md:text-4xl lg:text-[40px]  font-semibold mb-3 pb-4 px-4 leading-tight">
          {GrowthHeading}
        </h2>
        {GrowthDescription && (
          <p className="text-base md:text-lg mb-10">{GrowthDescription}</p>
        )}
          ...
          {slides.map((item, index) => (
            <SwiperSlide key={index}>
              <div className="flex flex-col items-center">
                ...
                {item.title && (
                  <h4 className="text-xl md:text-[22px] font-medium py-2">{item.title}</h4>
                )}
                <p
                  className="text-base md:text-[16px] font-normal pb-10"
